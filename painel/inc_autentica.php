<?php
include("includes/inc_database.php");
include("includes/inc_funcoeslib.php");
...
if (empty($login)) {
    ...
    $query_login = mysql_query("select * from tbl_sys_usuarios where login = '$login'") or die(mysql_error());
    $exibe_query_login = mysql_fetch_assoc($query_login);
    ...
    if ($total_login == 0) {
        ...
        mysql_query("insert into tbl_sys_log_ips values (NULL,'" . $dados_id . "','" . $data . "','" . $hora . "','" . $ip . "','Não')") or die(mysql_error());
    } else if ($senha == $dados_senha) {
        mysql_query("update tbl_sys_usuarios set datas = '" . $data . ";" . $dados_data . "' , ips = '" . $ip . ";" . $dados_ip . "' where login = '$login'") or die(mysql_error());

        $_SESSION['admin_usuario_logado'] = '1';
        $_SESSION['admin_usuario'] = $dados_nome;
        $_SESSION['admin_usuario_id'] = $dados_id;

        mysql_query("insert into tbl_sys_log_ips values (NULL,'" . $dados_id . "','" . $data . "','" . $hora . "','" . $ip . "','Sim')") or die(mysql_error());
        if (empty($_GET['p'])) {
